@echo off
REM Zmeri prefill (zpracovani promptu) podle jeho delky.
REM   scripts\prefill.bat D:\models\model.gguf [kontext] [tokenu]
...
cargo run --release --example prefill -p anvil-infrastructure --features engine-vulkan -- %*
set RC=%ERRORLEVEL%
endlocal & exit /b %RC%

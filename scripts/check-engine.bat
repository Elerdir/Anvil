@echo off
REM Prelozi crate i s feature `engine` (CPU-only) - overi, ze kod sedi na API
REM llama-cpp-2. Vulkan/Metal se tim netestuji, jen samotny engine.
...
where cmake >nul 2>&1
if errorlevel 1 ( echo [CHYBA] cmake nenalezen & exit /b 1 )
for %%V in (18 17 16) do (
    for %%E in (Community Professional Enterprise BuildTools) do (
...
echo [OK] prostredi pripraveno, spoustim cargo check
cargo check -p anvil-infrastructure --features engine %*
set RC=%ERRORLEVEL%
endlocal & exit /b %RC%

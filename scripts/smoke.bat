@echo off
REM Pusti smoke test proti skutecnemu modelu s Vulkan akceleraci.
REM   scripts\smoke.bat D:\models\model.gguf [kontext] [tokenu]
...
cargo run --release --example smoke -p anvil-infrastructure --features engine-vulkan -- %*
set RC=%ERRORLEVEL%
endlocal & exit /b %RC%
